...
experiments_:
    ...
    //addr1[offset] = 0xfb;
    volatile unsigned char* rr = oraclearr + PG_SIZE*0xfe;
    volatile unsigned char* rrshift = &addr1[offset];
    for(int rnd_i = 0; rnd_i < NUM_ROUNDS; ++rnd_i){
        memset(averg, 0x0, 256*sizeof(uint64_t));
        for(int i = 0; i < NUM_EXPR; ++i){
            int flag = 0;
	    unsigned char orr = 0xfe;
	    for(int speci = 0; speci < 100; ++speci){
		rrshift = &addr1[offset];
	    	if(speci == 99){
			*uncachedp = 1;
			//rrshift = NULL;
		}
